...
const POLICE_COLOR = '#2b4694';
const POLICE_LIGHT = '#ebf0ff';
...
export default function PoliceDashboard() {
  ...
  const allAlerts = alerts.filter(a => a.service === 'police' && a.responder?.id === user?.id);
  ...
  const tabs = [
    ...
    { value: 'resolved', label: 'Clôturés', count: allAlerts.filter(a => a.status === 'resolved').length },
  ];
  ...
  return (
    <PageLayout>
      <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* Header */}
        <div style={{ background: `linear-gradient(135deg, ${POLICE_COLOR}, #1a2f6b)`, borderRadius: 16, padding: 'clamp(20px,3vw,28px)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ fontSize: 'clamp(16px,3vw,22px)', fontWeight: 800, marginBottom: 3 }}>Police Nationale</h1>
              <p style={{ fontSize: 12, opacity: 0.75 }}>Gestion des incidents — Violence & Accidents</p>
            </div>
          </div>
        ...
        <div className="grid-4" style={{ gap: 14 }}>
          {[
            { label: 'Total incidents', value: allAlerts.length, icon: '📋', color: POLICE_COLOR, bg: POLICE_LIGHT },
            { label: 'En attente', value: allAlerts.filter(a => a.status === 'pending').length, icon: '⏳', color: '#d69e2e', bg: '#fffff0' },
            { label: 'En intervention', value: allAlerts.filter(a => a.status === 'in_progress').length, icon: '🚔', color: '#0096C7', bg: '#e0f7fa' },
            { label: 'Clôturés', value: allAlerts.filter(a => a.status === 'resolved').length, icon: '✅', color: '#48bb78', bg: '#f0fff4' },
          ].map(s => (
        ...
        <div style={{ display: 'flex', gap: 4, background: '#f7fafc', padding: 4, borderRadius: 12, width: 'fit-content', flexWrap: 'wrap' }}>
          {tabs.map(t => (
            <button key={t.value} onClick={() => setFilter(t.value)}
              ...
              {t.label}
              {t.count > 0 && <span style={{ background: filter === t.value ? POLICE_COLOR : '#e2e8f0', color: filter === t.value ? 'white' : '#718096', borderRadius: 10, padding: '1px 7px', fontSize: 11, fontWeight: 700 }}>{t.count}</span>}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#a0aec0' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 600 }}>Aucun incident assigné</p>
            <p style={{ fontSize: 13, color: '#a0aec0', marginTop: 4 }}>Les incidents vous seront assignés par votre superviseur</p>
          </div>
        ) : (
            ...
            {filtered.map(alert => (
              <div key={alert.id} className="card" style={{ padding: '16px 18px', cursor: 'pointer', borderLeft: `4px solid ${priorityColors[alert.priority]}`, transition: 'all 0.2s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ width: 46, height: 46, borderRadius: 12, background: POLICE_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>
      ...
      <Modal isOpen={!!selected} onClose={() => { setSelected(null); setNotes(''); }} title="Détail de l'incident" size="lg">
        {selected && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
              <div
                ...
                style={{ padding: 14, background: POLICE_LIGHT, borderRadius: 12, cursor: 'pointer', border: `1px solid transparent`, transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = POLICE_COLOR; e.currentTarget.style.boxShadow = `0 4px 14px ${POLICE_COLOR}25`; }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: POLICE_COLOR, background: '#fff', padding: '2px 7px', borderRadius: 6 }}>🗺️ Voir sur la carte</span>
                </div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {selected.status === 'assigned' && (
                <button onClick={() => handleStart(selected)} style={{ flex: 1, minWidth: 180, padding: 12, background: POLICE_COLOR, border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
