...
const FIRE_COLOR = '#c05621';
const FIRE_LIGHT = '#fffaf0';
...
export default function FireDashboard() {
  ...
  const allAlerts = alerts.filter(a => a.service === 'fire' && a.responder?.id === user?.id);
  ...
  const handleStart = (alert: Alert) => {
    ...
    toast.success('Intervention déclenchée !');
    setSelected(null);
  };
  ...
  const tabs = [
    { value: 'all', label: 'Toutes', count: allAlerts.length },
    { value: 'active', label: 'Actives', count: active.length },
    { value: 'assigned', label: 'Assignées', count: allAlerts.filter(a => a.status === 'assigned').length },
    { value: 'resolved', label: 'Terminées', count: allAlerts.filter(a => a.status === 'resolved').length },
  ];
  ...
  return (
    <PageLayout>
      <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* Header */}
        <div style={{ background: `linear-gradient(135deg, ${FIRE_COLOR}, #7b341e)`, borderRadius: 16, padding: 'clamp(20px,3vw,28px)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ fontSize: 'clamp(16px,3vw,22px)', fontWeight: 800, marginBottom: 3 }}>Sapeurs-Pompiers</h1>
              <p style={{ fontSize: 12, opacity: 0.75 }}>Gestion des incendies & inondations</p>
            </div>
          </div>
        ...
        <div className="grid-4" style={{ gap: 14 }}>
          {[
            { label: 'Total interventions', value: allAlerts.length, icon: '📋', color: FIRE_COLOR, bg: FIRE_LIGHT },
            { label: 'En attente', value: allAlerts.filter(a => a.status === 'pending').length, icon: '⏳', color: '#d69e2e', bg: '#fffff0' },
            { label: 'En cours', value: allAlerts.filter(a => a.status === 'in_progress').length, icon: '🚒', color: '#0096C7', bg: '#e0f7fa' },
            { label: 'Terminées', value: allAlerts.filter(a => a.status === 'resolved').length, icon: '✅', color: '#48bb78', bg: '#f0fff4' },
          ].map(s => (
        ...
        <div style={{ display: 'flex', gap: 4, background: '#f7fafc', padding: 4, borderRadius: 12, width: 'fit-content', flexWrap: 'wrap' }}>
          {tabs.map(t => (
            <button key={t.value} onClick={() => setFilter(t.value)}
              ...
              {t.label}
              {t.count > 0 && <span style={{ background: filter === t.value ? FIRE_COLOR : '#e2e8f0', color: filter === t.value ? 'white' : '#718096', borderRadius: 10, padding: '1px 7px', fontSize: 11, fontWeight: 700 }}>{t.count}</span>}
            </button>
          ))}
        </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#a0aec0' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 600 }}>Aucune intervention assignée</p>
            <p style={{ fontSize: 13, color: '#a0aec0', marginTop: 4 }}>Les interventions vous seront assignées par votre commandant</p>
          </div>
        ) : (
            ...
            {filtered.map(alert => (
              <div key={alert.id} className="card" style={{ padding: '16px 18px', cursor: 'pointer', borderLeft: `4px solid ${priorityColors[alert.priority]}`, transition: 'all 0.2s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ width: 46, height: 46, borderRadius: 12, background: FIRE_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>
      ...
      <Modal isOpen={!!selected} onClose={() => { setSelected(null); setNotes(''); }} title="Détail de l'intervention" size="lg">
        {selected && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
              <div
                ...
                style={{ padding: 14, background: FIRE_LIGHT, borderRadius: 12, cursor: 'pointer', border: `1px solid transparent`, transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = FIRE_COLOR; e.currentTarget.style.boxShadow = `0 4px 14px ${FIRE_COLOR}25`; }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: FIRE_COLOR, background: '#fff', padding: '2px 7px', borderRadius: 6 }}>🗺️ Voir sur la carte</span>
                </div>
              ...
              <div style={{ padding: 14, background: '#f7fafc', borderRadius: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#a0aec0', marginBottom: 6 }}>DÉCLARANT</div>
            ...
            {selected.status === 'in_progress' && (
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#4a5568', display: 'block', marginBottom: 6 }}>Rapport d'intervention (optionnel)</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Moyens engagés, situation actuelle..." rows={3}
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {selected.status === 'assigned' && (
                <button onClick={() => handleStart(selected)} style={{ flex: 1, minWidth: 180, padding: 12, background: FIRE_COLOR, border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                  🚒 Déclencher l'intervention
                </button>
              )}
              {selected.status === 'in_progress' && (
                <button onClick={() => handleResolve(selected)} style={{ flex: 1, minWidth: 180, padding: 12, background: '#48bb78', border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
